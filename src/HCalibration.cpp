...
std::unique_ptr<HParams> calibrate(const ffloat S,const std::list<options_chain>& market_data){
    ...
    if(dlevmar_der(get_prices_for_levmar, get_jacobian_for_levmar, p, x, 5, n_observations_cur, 100, opts, info, NULL, NULL, (void*) &adata)<0) throw std::runtime_error("levmar failed!");
    auto to_calib=std::unique_ptr<HParams>(new HParams({p[0],p[1],p[2],p[3],p[4]}));
    std::cout<<"v_0: "<<to_calib->v_0<<"\tv_m: "<<to_calib->v_m<<"\trho: "<<to_calib->rho<<"\tkappa: "<<to_calib->kappa<<"\tsigma: "<<to_calib->sigma<<'\n';
    //std::cout<<"x: "<<x<<"\tx2: "<<x2<<'\n';
