...
std::unique_ptr<HParams> calibrate(const ffloat S,const std::list<options_chain>& market_data){
    ...
    *to_calib={avg_iv*avg_iv, avg_iv*avg_iv, 0.04,1.0,1.0};
    for(std::list<options_chain>::const_iterator opts = market_data.begin(); opts != market_data.end(); opts++){
        ...
        auto current_SWIFT=std::make_unique<SWIFT>(*to_calib,S,*opts, SWIFT::get_m(*my_distr,static_cast<ffloat>(opts->days_to_expiry)));
    }
    return to_calib;
}
